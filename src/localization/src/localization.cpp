...
private:
    ...
    Eigen::Matrix4f       T_map_lidar_last_keyframe_ = Eigen::Matrix4f::Identity();
    ...
    // ---- Tuning parameters --------------------------------------------------
    const int   kAnchorScanTarget = 7;

    const float kKeyframeDistanceM = 0.20f;

    // matches typically score < 0.05; open-area junk scores > 0.2.
    const float kIcpFitnessThreshold = 0.10f;
    const float kIcpInterpolationAlpha = 0.10f;
	const float distanceToRunICP = 0.10f; // only run ICP if we've traveled at least this much since the last keyframe
	const float maxAngularVelForGoodScan = 0.3f; // only run ICP if angular velocity is below this (rads/s)
    ...
    void scanCallback(const sensor_msgs::msg::LaserScan::SharedPtr msg)
    {
        ...
        Eigen::Matrix4f T_map_lidar_guess = T_map_odom_ * T_odom_base_ * T_base_lidar_;

        // --- Build submap (all keyframes for this small environment) ----------
        CloudT::Ptr submap = buildSubmap();
        publishCloud(submap, "map", pub_ref_cloud_);
        {
            CloudT::Ptr cloud_at_guess(new CloudT());
            pcl::transformPointCloud(*scan_cloud, *cloud_at_guess, T_map_lidar_guess);
            publishCloud(filterCloud(cloud_at_guess), "map", pub_scan_guess_);
        }
		...
		if (distance_travelled_last_icp_ > distanceToRunICP && std::abs(ang_vel_) < maxAngularVelForGoodScan) {
			...
			if (score > kIcpFitnessThreshold) {
				RCLCPP_WARN(this->get_logger(),
							"ICP fitness %.4f > threshold %.4f, skipping update.",
							score, kIcpFitnessThreshold);
				return;
			}
			RCLCPP_DEBUG(this->get_logger(), "ICP converged, score: %.4f", score);

			distance_travelled_last_icp_ = 0.0f;
			
			// --- Update T_map_odom using ICP-corrected pose ----------------------
			T_map_lidar_icp  = icp.getFinalTransformation();
			Eigen::Matrix4f T_map_base_icp   = T_map_lidar_icp * T_base_lidar_.inverse();
			Eigen::Matrix4f T_map_odom_icp   = T_map_base_icp * T_odom_base_.inverse();
			T_map_odom_ = interpolateTransform(T_map_odom_, T_map_odom_icp, kIcpInterpolationAlpha);
			
			if (shouldAddKeyframe(T_map_lidar_icp)) addKeyframe(scan_cloud, T_map_lidar_icp);
    ...
    void accumulateAnchor(const CloudT::Ptr & scan_lidar, const Eigen::Matrix4f & T_map_lidar)
    {
        if (keyframes_.empty()) {
            ...
            T_map_lidar_last_keyframe_ = T_map_lidar;
        }
        ...
        *keyframes_[0].cloud_in_map += transformed;
        ...
        // On the last accumulation pass, filter the whole anchor cloud
        if (anchor_scan_count_ == kAnchorScanTarget - 1) {
            keyframes_[0].cloud_in_map = filterCloud(keyframes_[0].cloud_in_map);
        }
    }
    ...
    bool shouldAddKeyframe(const Eigen::Matrix4f & T_map_lidar_now) const
    {
        ...
        Eigen::Vector3f dt = T_map_lidar_now.block<3,1>(0,3)
                           - T_map_lidar_last_keyframe_.block<3,1>(0,3);
    ...
    void addKeyframe(const CloudT::Ptr & scan_lidar, const Eigen::Matrix4f & T_map_lidar)
    {
        ...
        cloud_map = filterCloud(cloud_map);
